...
import { nativeOcr } from './native-ocr.service';
import { HybridOcrService } from './hybrid-ocr.service';
import { ocrTaskQueue } from '../task-queue.service';
import { ocrSpaceProvider } from './online-ocr-space.service';
import type { OcrResult, OcrRequest } from '@shared/types';

// Initialize hybrid OCR service
// Use Native OCR as the primary "offline" provider for Windows
// Fallback to Tesseract if needed (though we are replacing it here)
const hybridOcr = new HybridOcrService(nativeOcr, {
  mode: 'offline', // Force offline mode since we use native
  preferOffline: true,
  onlineTimeout: 10000,
  enableFirewallDetection: false,
});

// Register free online provider (OCR.Space) - requires API key (free tier available)
hybridOcr.registerOnlineProvider(ocrSpaceProvider);
...
class OcrService {
  ...
  async processImage(
    ...
    const ocrEngine = localStorage.getItem('ocr_engine') || 'offline';
    if (ocrEngine === 'online') {
  ...
  async initialize(language = 'eng'): Promise<void> {
    try {
      ...
      // Do not block app startup if initialization fails; log and continue
      console.warn('OCR initialize skipped/failed, continuing:', error);
    }
  }
