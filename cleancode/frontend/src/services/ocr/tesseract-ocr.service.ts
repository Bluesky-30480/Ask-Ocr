...
export class TesseractOcrService {
  ...
  async initialize(language = 'eng'): Promise<void> {
    ...
    try {
      ...
      // Create new worker with language
      ...
      this.worker = await createWorker(language, 1, {
        ...
        cachePath: 'tessdata', // Cache data locally
      });
  ...
  async recognize(
    ...
    // Update logger for this specific request if progress callback provided
    if (onProgress) {
      // We can't easily update the logger of an existing worker in v5 without recreating it
      // But we can try to attach a listener if the worker exposes it, or just rely on the global logger
      // For now, we'll just use the console log from the global logger
    }
