...
import { universalAI } from '../ai/universal-ai.service';
...
export class MathFormulaOCRService {
   ...
   * Now uses AI to repair broken OCR math
   */
  async detectFormulas(text: string): Promise<MathOCRResult> {
    // 1. Quick heuristic check
    if (!this.hasMathematicalContent(text)) {
      return { text, formulas: [], hasFormulas: false };
    }

    // 2. Try to repair with AI
    try {
        const repairedText = await this.repairMathWithAI(text);
        
        // 3. Extract formulas from the repaired text (looking for $...$ or $$...$$)
        const formulas = this.extractLatexFormulas(repairedText);
        ...
        console.warn('AI Math repair failed, falling back to regex', e);
  ...
  private async repairMathWithAI(text: string): Promise<string> {
      const prompt = `
You are a mathematical OCR correction engine. 
The following text was extracted from an image but contains broken math formulas (e.g. "x2" instead of "x^2", "V" instead of square root).
Your task is to:
1. Identify the mathematical parts.
2. Convert them to proper LaTeX format enclosed in $...$ (inline) or $$...$$ (display).
3. Keep the non-math text exactly as is.
4. Output ONLY the corrected text. Do not add any conversational filler.

Input Text:
"${text}"
`;
      // Use the default configured model (which might be offline/local)
      const response = await universalAI.chat(prompt, 'local', undefined, []);
      return response.text.trim();
  }

  private extractLatexFormulas(text: string): MathFormula[] {
      ...
      // Match $...$ (inline) - be careful not to match inside $$...$$
      // Simple approach: remove display formulas first or use a complex regex.
      // For simplicity, we'll just match $...$ that wasn't already caught.
      // Actually, let's just iterate again and check overlap.
      const inlineRegex = /\$([^$]+)\$/g;
      while ((match = inlineRegex.exec(text)) !== null) {
          // Check if this range overlaps with any existing display formula
          const start = match.index;
          const end = match.index + match[0].length;
          const overlaps = formulas.some(f => 
              (start >= f.position.start && start < f.position.end) ||
              (end > f.position.start && end <= f.position.end)
          );
