...
export class ShortcutManager {
  ...
  async initialize(): Promise<void> {
    ...
    this.unlistenFn = await listen<string>('shortcut-triggered', (event: any) => {
      ...
      console.log('[ShortcutManager] Received shortcut-triggered event:', shortcutId);
      console.log('[ShortcutManager] Registered listeners:', Array.from(this.listeners.keys()));
      const callback = this.listeners.get(shortcutId);
      if (callback) {
        console.log('[ShortcutManager] Executing callback for:', shortcutId);
        callback();
      } else {
    ...
    console.log('[ShortcutManager] Initialized and listening for shortcuts');
  }
  ...
  async register(
    ...
    try {
      console.log(`[ShortcutManager] Registering shortcut: ${id} with accelerator: ${accelerator}`);
      ...
      if (result.success) {
        ...
        console.log(`[ShortcutManager] ✓ Shortcut registered successfully: ${id} (${accelerator})`);
        console.log(`[ShortcutManager] Total registered shortcuts: ${this.listeners.size}`);
      } else {
        console.error(`[ShortcutManager] ✗ Failed to register shortcut: ${result.error}`);
      }
