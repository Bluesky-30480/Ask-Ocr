  ...
  async sendStreamingRequest(
    ...
    if (request.forceProvider) {
      ...
      const selection = await priorityStrategy.selectProvider({
        taskType,
        textLength: request.query.length + (request.ocrText?.length || 0),
        requiresNetwork: template === 'ocr_qa',
      });
      provider = selection.provider;
      model = selection.model || 'default';
    }
    ...
    try {
      ...
      if (provider === 'local' || provider === 'custom') {
        return await this.streamToOllama(aiRequest, onChunk);
      } else {
        // Fallback to non-streaming for other providers for now
        // TODO: Implement streaming for cloud providers
        const response = await this.sendToProvider(provider as any, aiRequest);
    ...
    while (true) {
      ...
      for (const line of lines) {
        ...
        try {
          ...
          // Handle DeepSeek R1 style thinking tags <think>...</think>
          // This is a simple parser, might need robustness for split tags
          let contentToStream = content;
          let thinkingToStream = '';

          if (content.includes('<think>')) {
            inThinkingBlock = true;
            const parts = content.split('<think>');
            contentToStream = parts[0];
            // If there's content after <think>, it's thinking
            if (parts[1]) {
                if (parts[1].includes('</think>')) {
                    const thinkParts = parts[1].split('</think>');
                    thinkingToStream = thinkParts[0];
                    contentToStream += thinkParts[1];
                    inThinkingBlock = false;
                } else {
                    thinkingToStream = parts[1];
                }
            }
          } else if (content.includes('</think>')) {
             const parts = content.split('</think>');
             thinkingToStream = parts[0];
             contentToStream = parts[1];
             inThinkingBlock = false;
          } else if (inThinkingBlock) {
             thinkingToStream = content;
             contentToStream = '';
          }

          if (contentToStream || thinkingToStream) {
              fullContent += contentToStream;
              fullThinking += thinkingToStream;
              onChunk({ 
                  content: contentToStream, 
                  thinking: thinkingToStream,
                  done: false 
              });
          }
          ...
          if (data.done) {
            ...
            return {
              ...
              provider: 'local',
              model: request.model || 'llama2',
              template: 'ai_assistant', // Default
              confidence: 1.0,
              thinkingProcess: fullThinking,
              usage: {
...
Selected text: "${context.selectedText.substring(0, 200)}${
   ...
   */
  async listModels(provider: string): Promise<{ id: string; name: string }[]> {
    try {
      switch (provider) {
        case 'openai':
        ...
        case 'gemini':
          ...
          const geminiRes = await fetch(`https://generativelanguage.googleapis.com/v1beta/models?key=${this.config.geminiApiKey}`);
          if (!geminiRes.ok) throw new Error('Failed to fetch Gemini models');
        ...
        case 'deepseek':
          if (!this.config.deepseekApiKey) return [];
          const deepseekRes = await fetch('https://api.deepseek.com/models', {
            headers: { 'Authorization': `Bearer ${this.config.deepseekApiKey}` }
          });
          if (!deepseekRes.ok) throw new Error('Failed to fetch DeepSeek models');
          const deepseekData = await deepseekRes.json();
          return deepseekData.data.map((m: any) => ({ id: m.id, name: m.id }));

        case 'grok':
          if (!this.config.grokApiKey) return [];
          const grokRes = await fetch('https://api.x.ai/v1/models', {
            headers: { 'Authorization': `Bearer ${this.config.grokApiKey}` }
          });
          if (!grokRes.ok) throw new Error('Failed to fetch Grok models');
          const grokData = await grokRes.json();
          return grokData.data.map((m: any) => ({ id: m.id, name: m.id }));
