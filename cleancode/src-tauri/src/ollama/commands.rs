...
#[tauri::command]
pub async fn ollama_generate_stream(window: Window, model: String, prompt: String, request_id: String) -> Result<(), String> {
    let client = Client::new();
    let request = GenerateRequest {
        model,
        prompt,
    ...
    while let Some(item) = stream.next().await {
        ...
        for line in chunk_str.lines() {
            ...
            if let Ok(response) = serde_json::from_str::<GenerateResponse>(line) {
                if let Some(error) = response.error {
                    let _ = window.emit("ollama-stream-error", serde_json::json!({
                        "id": request_id,
                        "error": error
                    }));
                    return Err(error);
                ...
                let _ = window.emit("ollama-stream-response", serde_json::json!({
                    "id": request_id,
